...
namespace Teltonika.Codec.Devices
{
    public class Codec16
    {
        ...
        public Codec16(ReverseBinaryReader reader)
        {
            if (reader == null)
            {
        ...
        private AvlData DecodeAvlData()
        {
            ...
            var originType = _reader.ReadByte();

            var propertiesCount = _reader.ReadByte();
            // IO Element Properties decoding
            var ioProperties = DecodeIoProperties();

            var ioElement = IoElement.Create(eventId, propertiesCount, ioProperties, originType);
